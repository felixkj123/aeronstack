...
start_init () {
	...
	###nova
	openstack user create --domain default --password $SERVICE_PASSWORD nova; aeron_cmd_stat $aeron_startinit_retval $machine start_init_placement

	openstack role add --project service --user nova admin; aeron_cmd_stat $aeron_startinit_retval $machine start_init_placement

	openstack service create --name nova --description "OpenStack Compute" compute; aeron_cmd_stat $aeron_startinit_retval $machine start_init_placement

	openstack endpoint create --region RegionOne compute public http://controller:8774/v2.1; aeron_cmd_stat $aeron_startinit_retval $machine start_init_placement

	openstack endpoint create --region RegionOne compute internal http://controller:8774/v2.1; aeron_cmd_stat $aeron_startinit_retval $machine start_init_placement

	openstack endpoint create --region RegionOne compute admin http://controller:8774/v2.1; aeron_cmd_stat $aeron_startinit_retval $machine start_init_placement
}
